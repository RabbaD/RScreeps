		...
		for (let role in roleOccupancy) {
			...
			if (needed > 0) {
				stringReport.push('| ' + `${role}:`.padRight(padLength) +
								  `${Math.floor(100 * current / needed)}%`.padLeft(4) + `${current}`.padLeft(3) + ` / ${needed}`.padLeft(3));
			}
		}
		let stateString = this.colony.stage == ColonyStage.Larva ? 'Larva' : this.colony.stage == ColonyStage.Adult ? 'Adult' : 'Pupa'
		stringReport.push(`State: ${stateString}`)

		// for (let priority in this.overlords) {
		// 	if (!this.overlords[priority]) continue;
		// 	for (let overlord of this.overlords[priority]) {
		// 		stringReport.push(`Overlord: ${overlord.name}`)
		// 	}
		// }
